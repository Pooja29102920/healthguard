...
DB_PATH     = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'healthguard.db')
SECRET_FILE = os.path.join(os.path.dirname(__file__), '.secret')
STATIC_DIR  = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'frontend'))
...
SSE_TIMEOUT   = 15               # heartbeat every 15s — keeps cloud proxies alive
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _api_get(self, path, qs):
        ...
        if path == '/api/ping':
            return self._json(200, {'ok': True, 'status': 'healthy'})
        ...
        # /api/pairing-code or /api/pairing/my-code  (patient generates their pairing code)
        if path in ('/api/pairing-code', '/api/pairing/my-code'):
            if not claim: return self._json(401, {'error': 'Unauthorized'})
            with get_db() as db:
    ...
    def _api_post(self, path):
        ...
        # /api/pairing-code  (patient generates pairing code — POST version)
        if path in ('/api/pairing-code', '/api/pairing/my-code'):
            if not claim: return self._json(401, {'error': 'Unauthorized'})
            with get_db() as db:
                pt = db.execute('SELECT * FROM patients WHERE user_id=?', (claim['uid'],)).fetchone()
                if not pt:
                    ...
                    pid = 'pr' + claim['uid']
                    db.execute(
                        'INSERT OR IGNORE INTO patients(id,user_id,caregiver_id,name,added_at) VALUES(?,?,NULL,?,?)',
                        (pid, claim['uid'], claim.get('name','Patient'), now_iso())
                    )
                    pt = db.execute('SELECT * FROM patients WHERE user_id=?', (claim['uid'],)).fetchone()
                if not pt: return self._json(404, {'error': 'Patient record not found'})
                db.execute('UPDATE pairing_codes SET used=1 WHERE patient_id=? AND used=0', (pt['id'],))
